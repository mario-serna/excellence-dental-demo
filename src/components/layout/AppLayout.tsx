...
const adminItems = [{ label: "Equipo", path: "/admin/users", icon: UserCog }];
...
export default function AppLayout() {
  ...
  return (
    <div className="flex min-h-screen w-full bg-background">
      ...
      <aside
        className={cn(
          "hidden md:flex flex-col border-r border-border bg-card transition-all duration-300 shrink-0 fixed left-0 top-0 h-screen z-30",
          "w-16 lg:w-64",
          collapsed && "lg:w-16",
        )}
      >
        <div className="h-16 flex items-center justify-center lg:justify-start px-4 border-b border-border overflow-hidden">
          ...
          <span
            className={cn(
              "font-semibold text-lg text-primary ml-2 hidden lg:inline whitespace-nowrap",
              collapsed && "lg:hidden",
            )}
          >
            Excellence
        ...
        <nav className="flex-1 py-4 space-y-1 px-2">
          {items.map((item) => (
            ...
            >
              ...
              <span
                className={cn("hidden lg:inline", collapsed && "lg:hidden")}
              >
                {item.label}
              </span>
            </NavLink>
        ...
        <div className="border-t border-border p-3">
          ...
          <Button
            ...
            title="Cerrar sesión"
            className={cn(
              "w-full gap-2 justify-center lg:justify-start",
              !collapsed && "lg:justify-start",
            )}
          >
            <LogOut className="h-4 w-4" />
            <span className={cn("hidden lg:inline", collapsed && "lg:hidden")}>
              Cerrar sesión
            </span>
          </Button>
        </div>
      ...
      <div
        className="flex-1 flex flex-col min-h-screen min-w-0 md:ml-16 lg:ml-64 transition-all duration-300"
        style={{ marginLeft: collapsed ? "4rem" : undefined }}
      >
        {/* Mobile top bar */}
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-card border-t border-border flex items-center justify-around h-16 z-50">
          {navItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  "flex flex-col items-center gap-1 py-2 px-3 min-w-[44px] min-h-[44px] justify-center",
          ...
          <button
            ...
            className="flex flex-col items-center gap-1 py-2 px-3 min-w-[44px] min-h-[44px] justify-center text-muted-foreground"
          >
            <Menu className="h-5 w-5" />
